...
DRIVERS = list(DRIVER_PASSWORDS.keys())
...
class Order(db.Model):
    ...
    def as_dict(self):
        payment_record = PaymentRecord.query.filter_by(order_id=self.id).first()
        return {
            ...
            "payment_status": payment_record.status if payment_record else ("Cash" if self.payment == "Cash" else "En attente"),
            "transaction_id": payment_record.transaction_id if payment_record else "",
...
class PaymentRecord(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    order_id = db.Column(db.Integer, db.ForeignKey("order.id"), unique=True, nullable=False, index=True)
    method = db.Column(db.String(50), nullable=False)
    transaction_id = db.Column(db.String(120), default="")
    status = db.Column(db.String(50), default="En attente", index=True)
...
def login():
    ...
    if request.method == "POST":
        ...
        if role == "admin" and ADMIN_PASSWORD and hmac.compare_digest(username, ADMIN_USER) and hmac.compare_digest(password, ADMIN_PASSWORD):
            session.clear(); session["role"] = "admin"; session["name"] = "Admin"
            return redirect("/admin")
        if role == "driver" and username in DRIVER_PASSWORDS and DRIVER_PASSWORDS[username] and hmac.compare_digest(password, DRIVER_PASSWORDS[username]):
...
def add_product():
    ...
    p = Product(name=name, category=str(d.get("category", "other")), description=str(d.get("description", "Ajoute pa Admin")),
...
def edit_product(pid):
    ...
    d = request.get_json(silent=True) or {}
    if "name" in d and str(d.get("name", "")).strip(): p.name = str(d["name"]).strip()
...
def create_order():
    ...
    o = Order(code=code, customer=customer, phone=phone, address=address,
              ...
              payment=payment, order_type=str(d.get("order_type", "Livraison")),
    ...
    if payment in ("MonCash", "NatCash"):
        db.session.add(PaymentRecord(order_id=o.id, method=payment, transaction_id=transaction_id, status="En attente"))
    db.session.commit()
...
def patch_order(oid):
    ...
    if role == "admin":
        ...
        if "driver" in d:
            driver = str(d["driver"])
            if driver and driver not in DRIVERS: return jsonify({"error": "invalid_driver"}), 400
            o.driver = driver
...
@app.patch("/api/orders/<int:oid>/payment")
@admin_required
def verify_payment(oid):
    o = db.session.get(Order, oid)
    if not o: return jsonify({"error": "not_found"}), 404
    if o.payment == "Cash": return jsonify({"error": "cash_payment"}), 400
    p = PaymentRecord.query.filter_by(order_id=o.id).first()
    if not p: return jsonify({"error": "payment_not_found"}), 404
    d = request.get_json(silent=True) or {}
    status = str(d.get("status", "Konfime"))
    if status not in ("Konfime", "Rejte", "En attente"): return jsonify({"error": "invalid_status"}), 400
    p.status = status
    p.verified_at = datetime.utcnow() if status == "Konfime" else None
    db.session.commit()

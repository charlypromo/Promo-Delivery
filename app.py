...
def seed_ads():
    ...
    db.session.add(Ad(
        ...
        image_url="", target_url="", active=True
    ))
    db.session.commit()
    ...
    if request.method == "POST":
        ...
        if role == "customer":
            ...
            if user and check_password_hash(user.password_hash, password):
                session.clear(); session["role"] = "customer"; session["customer_id"] = user.id; session["name"] = user.full_name
                return redirect("/")
...
def add_ad():
    ...
    ad = Ad(title=title, description=str(d.get("description", "")).strip(),
            image_url=str(d.get("image_url", "")).strip(), target_url=str(d.get("target_url", "")).strip(),
